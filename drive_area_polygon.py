...
from drive_area_detection.msg import CnnOutput, ClusteringOutput
...
class LDClusteringNode:
    ...
    def points_received_callback(self, points_data):
        ...
        try:
            if self.vis_topic:
                lanes_image = np.zeros((576,1024,3))
                ...
                cv2.imshow("Lanes", cv2.resize(lanes_image, (1024//2,576//2), cv2.INTER_NEAREST))
                cv2.waitKey(1)
    ...
    def __init__(self):
        ...
        try:
            ...
            self.vis_topic = rospy.get_param('vis_topic', True)
            self.vis_topic_name = rospy.get_param('Cluster/polygon', True)
        ...
        try:        
            ...
            rospy.Subscriber('Drive/cluster_input', numpy_msg(CnnOutput), self.points_received_callback)
            rospy.spin()
        except Exception as e:
