...
class hardnet_culane(nn.Module):
    def __init__(self, n_classes=19):
        super(hardnet_culane, self).__init__()
        ...
        self.clsConv = nn.Sequential(
            ConvLayer(320, 160,  kernel=3),
            ConvLayer(160, 80,  kernel=3),
            ConvLayer(80, 40,  kernel=3),
            ConvLayer(40, 4,  kernel=3),
            nn.Sigmoid()
    ...
    def forward(self, x):
        ...
        out = F.interpolate(
                            ...
                            mode="bilinear",
                            align_corners=True)
        return out ,cls.max(2)[0].max(2)[0]
